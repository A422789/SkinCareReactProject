...
export function CollectionPage({ eyebrow, title, description, products, accent = 'gold' }) {
  return (
    <main style={{ margin: '0 auto', maxWidth: '72rem', padding: '3.5rem 1rem' }} className="collection-main">
      ...
      {products.length === 0 ? (
        <p style={{ padding: '5rem 0', textAlign: 'center', color: 'var(--muted-foreground)' }}>No products found.</p>
      ) : (
      ...
      <style>{`
        ...
        @media (min-width: 768px) {
          ...
          .products-flex { flex-direction: row; flex-wrap: wrap; gap: 1.25rem; }
          .products-item { width: calc(25% - 0.95rem); }
        }
      `}</style>
    </main>
  )
